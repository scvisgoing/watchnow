...
class HostViewTestCase(TestCase):
    ...
    def test_create_a_host(self):
        ...
        # settings 中若有定義 DEFAULT_PERMISSION_CLASSES 則會導致 403 錯誤 {'detail': 'Authentication credentials were not provided.'}
    ...
    def test_get_many_hosts(self):
        ...
        response = self.client.post(reverse('host-list'), self.host_data, format="json")
    ...
    def test_update_host(self):
        ...
        change_name = {'name': 'Wow-DEV'} # change_name = {'name': 'Wow-DEV', 'ipv4':'10.62.41.100'} 若用 put 就得塞這樣
        res = self.client.patch(reverse('host-detail', kwargs={'pk': host.id}), change_name, format='json')
    ...
    def test_delete_host(self):
        ...
        host = Host.objects.get()
        res = self.client.delete(reverse('host-detail', kwargs={'pk': host.id}), format='json', follow=True)
        # 成功的話 res.data = None
