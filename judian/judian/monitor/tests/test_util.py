...
class TestUtil(unittest.TestCase):
    ...
    def test_config(self):
        ...
        self.assertEqual(
            util.get_config_option(config_options, 'test_[int]', required_type='[int]'), [1, 2, 3])
        self.assertEqual(
            util.get_config_option(config_options, 'test_[str]', required_type='[str]'),
            ['a', 'b', 'c'])
        for i in list(range(1, 5)):
            self.assertEqual(
                util.get_config_option(config_options, f'test_bool{i}', required_type='bool'), True)
        self.assertEqual(
            util.get_config_option(config_options, 'test_bool5', required_type='bool'), False)
        # Raise test
        with self.assertRaises(ValueError):
            util.get_config_option(['not a dict'], '') # 第一個參數若非字典將引發例外
        # 傳入一個不存在的key卻又指定它為必要則引發例外
        with self.assertRaises(ValueError):
            util.get_config_option(config_options, 'missing_key', required=True)
        # 傳入個字串卻要將它轉成 int 會引發例外
        with self.assertRaises(ValueError):
        ...
        # 驗證 minimum
        with self.assertRaises(ValueError):
            util.get_config_option(config_options, 'test_int', required_type='int', minimum=4)
        # 驗證 maximum
        with self.assertRaises(ValueError):
            util.get_config_option(config_options, 'test_int', required_type='int', maximum=2)
        # 試圖'a,b,c'轉成 int list
        with self.assertRaises(ValueError):
            util.get_config_option(config_options, 'test_[str]', required_type='[int]')
        # 測試 allowed_values for list
        with self.assertRaises(ValueError):
            util.get_config_option(config_options, 'test_[str]', required_type='[str]',
                                   allowed_values=['d'])
        # 測試 allowed_values for string
        with self.assertRaises(ValueError):
            util.get_config_option(config_options, 'test_string',
        ...
        with self.assertRaises(ValueError):
            util.get_config_option({'empty_string': ''}, 'empty_string',
                                   required_type='str', allow_empty=False)

        with self.assertRaises(util.MonitorConfigurationError):
            util.get_config_option('not a dict', "doesn't matter",
                                   exception=util.MonitorConfigurationError)
    ...
    # test util.JSONEncoder class by json_dumps(data)
    # test util.JSONDecoder class by json_loads(string)
    # test util.subclass_dict_handler(mod, base_cls)
